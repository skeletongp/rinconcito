        ...
        @foreach ($carts as $cart)
            <div class="flex items-center space-x-2 my-2">
                <div
                    ...
                    <form action="{{ route('charts.destroy', $cart) }}" method="POST" id="c{{$cart->id}}">
                        @method('delete')
                        @csrf
                        <button data-label="¿Eliminar del carrito?" class="confirm" form="c{{$cart->id}}"><span
...
<div class="flex items-center {{$carts->count()?:'hidden'}} justify-between space-x-3 ">
    ...
    <form action="{{ route('charts.empty') }}" method="POST" id="formVaciar">
        @method('delete')
        @csrf
