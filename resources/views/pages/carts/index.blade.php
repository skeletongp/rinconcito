    ...
    <div class="max-w-3xl mx-auto bg-white p-4 rounded-xl">
        <div class="flex flex-col max-w-3xl p-6 space-y-4 sm:p-10 dark:bg-coolGray-900 dark:text-coolGray-100">
                            ...
                            <div class="flex w-full space-x-2 sm:space-x-4">
                                ...
                                <div class="flex flex-col justify-between w-full pb-4">
                                    ...
                                    <div class="flex text-sm divide-x">
                                        <div
                                            class="flex justify-between items-center w-full py-1 pl-0 space-x-1">
                                            <form class="w-full hover:text-red-400" action="{{ route('charts.destroy', $cart) }}"
                                                method="POST">
                                                @method('delete')
                                                @csrf
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="flex space-x-2 items-center">
                                                            <x-input name="cant" type="number" value="{{ $cart->cant }}"
                                                                ...
                                                                <button class="w-6 h-6 flex items-center justify-center rounded-full bg-green-400 text-white"><span class="fas fa-check"></span></button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </form>
            ...
            <div class="
                ...
                <p>Subtotal:
                    <input type=" text" disabled class="font-semibold money w-16 text-right" id="spSubtotal"
                        value="${{ number_format($carts->sum('total'), 2) }}" />
                </p>
                <p>Desc:
                    <input type="text" disabled class="font-semibold  w-16 text-right money" id="spDesc"
                        value="${{ number_format(0, 2) }}" />
                </p>
                <p>Total:
                    <input type="text" disabled class="font-semibold money w-16 text-right" id="spTotal"
                        value="${{ number_format($carts->sum('total'), 2) }}" />
                </p>
            </div>
            <div>
